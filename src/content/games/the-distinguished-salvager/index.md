...
For the colour posterization I wrote a shader that accepts a sampler2D uniform to be used as a color palette, and sets it's color to the closest matching color found in the palette. This shader is applied to each level's SubViewport, which allows for the different types of level to have their own palette. If I wanted to use the color palette authentically I could have set the filter mode on the palette to closest, but I opted instead to use linear filtering with more steps than there were colors in the palette to get control over how many in-between colors there are between defined colors in the palette. I found this to look better, since I have less direct control over each pixel on the screen in 3D than in 2D and the filter would sometimes pick unnatural colors for certain objects (for example, without this the water would occasionally be green since it was closer in distance than the nearest blue in the palette). The color pallettes  are all from [lospec](https://lospec.com/), specifically:
...
To accomplish the splotchy screen transitions, I wrote a shader that takes the vertex color of a UI element and applies that with a mask to determine if it should be transparent: if the vertex color alpha is less than the mask's value the pixel is completely transparent, otherwise it is completely opaque. I then apply a noise texture as the mask, so that the previous image fades away in the shape of the noise texture. I can then animate this fade by changing the vertex alpha of the UI element by changing it's color in code the same way as any normal fade. To make this work as a level transition, and to ensure that there are never more objects loaded than needed, before loading I take a screenshot of the game and display it in front of the whole game. I then unload the previous level and load the new one, and then fade out the screenshot. This ensures that even during screen transitions there is only ever one level system loaded.
