...
Since this is a game build for the Nintendo DS, I used a combination of NFLib and NitroEngine. NFLib was used to handle all the 2D assets on the secondary screen, and the primary screen is run with NitroEngine in 3D mode. During gameplay the primary screen is the top screen, but to achieve the pre-rendered title and game over screens as well as the rotating earth on the title and game over screens, I swap the primary to be the bottom screen. To parse the assets into the Nintendo DS format, I used the python script I had made for [Love Crafters](@/games/love-crafters.md) to convert various assets into their required formats.
